...
class Raw_Data_validation:
    ...
    def valuesFromSchema(self):
        ...
        try:
            ...
            file = open(
                "Training_Logs/valuesfromSchemaValidationLog.txt", 'a+')
            ...
            file = open(
                "Training_Logs/valuesfromSchemaValidationLog.txt", 'a+')
            self.logger.log(
                file, "KeyError:Key value error incorrect key passed")
            file.close()
            raise KeyError
    ...
    def deleteExistingGoodDataTrainingFolder(self):
        ...
        try:
            ...
            if os.path.isdir(path + 'Good_Raw/'):
                ...
                file = open("Training_Logs/GeneralLog.txt", 'a+')
                self.logger.log(
                    file, "GoodRaw directory deleted successfully!!!")
                file.close()
        except OSError as s:
            file = open("Training_Logs/GeneralLog.txt", 'a+')
    ...
    def moveBadFilesToArchiveBad(self):
        """
        This method deletes the directory made  to store the Bad Data  after moving the data in an archive folder. We archive the bad
        ...
        try:
            ...
            if os.path.isdir(source):
                ...
                if not os.path.isdir(path):
                    os.makedirs(path)
                dest = 'TrainingArchiveBadData/BadData_' + \
                    str(date)+"_"+str(time)
                ...
                if os.path.isdir(path + 'Bad_Raw/'):
                    shutil.rmtree(path + 'Bad_Raw/')
                self.logger.log(
                    file, "Bad Raw Data Folder Deleted successfully!!")
                file.close()
        except Exception as e:
            file = open("Training_Logs/GeneralLog.txt", 'a+')
            self.logger.log(
                file, "Error while moving bad files to archive:: %s" % e)
            file.close()
            raise e
    ...
    def validationFileNameRaw(self, regex, LengthOfDateStampInFile, LengthOfTimeStampInFile):
        ...
        try:
                ...
                if (re.match(regex, filename)):
                    ...
                    if len(splitAtDot[1]) == LengthOfDateStampInFile:
                        if len(splitAtDot[2]) == LengthOfTimeStampInFile:
                            ...
                            self.logger.log(
                                f, "Valid File name!! File moved to GoodRaw Folder :: %s" % filename)

                        else:
                            shutil.copy("Training_Batch_Files/" + filename,
                                        "Training_Raw_files_validated/Bad_Raw")
                            self.logger.log(
                                f, "Invalid File Name!! File moved to Bad Raw Folder :: %s" % filename)
                    else:
                        shutil.copy("Training_Batch_Files/" + filename,
                                    "Training_Raw_files_validated/Bad_Raw")
                        self.logger.log(
            ...
            f = open("Training_Logs/nameValidationLog.txt", 'a+')
            self.logger.log(
                f, "Error occured while validating FileName %s" % e)
            f.close()
            raise e
        ...
        try:
            ...
            for file in listdir('Training_Raw_files_validated/Good_Raw/'):
                ...
                if csv.shape[1] == NumberofColumns:
                    ...
                    shutil.move("Training_Raw_files_validated/Good_Raw/" +
                                file, "Training_Raw_files_validated/Bad_Raw")
                    self.logger.log(
                        f, "Invalid Column Length for the file!! File moved to Bad Raw Folder :: %s" % file)
        ...
        try:
            ...
            for file in listdir('Training_Raw_files_validated/Good_Raw/'):
                ...
                if count == 0:
                    ...
                    csv.to_csv("Training_Raw_files_validated/Good_Raw/" +
